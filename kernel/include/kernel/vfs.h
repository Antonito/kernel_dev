...
struct fs_node {
  char name[FD_MAXFILENAME]; // The filename.
  uint32_t mask;             // The permissions mask.
  uint32_t uid;              // The owning user.
  uint32_t gid;              // The owning group.
  uint32_t flags;            // Includes the node type. See #defines above.
  uint32_t
      inode; // This is device-specific - provides a way for a filesystem to
             // identify files.
...
struct dirent {
  char name[FD_MAXFILENAME]; // Filename.
